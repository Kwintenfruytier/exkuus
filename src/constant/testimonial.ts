export const quotes = [
    {
        name: '"Je krijgt letterlijk alle tools, scripts, bonussen en dan kom je ook nog eens terecht in een hechte familie die je motiveert, inspireert, elke dag opnieuw!"',
        name2: '- Sankara D',
        bg: '#FFDF2B',
        color: '#000000',
    },

    {
        name: '"Ik heb er genoeg aandacht aan geschonken. Nu heb ik een duidelijk beeld gekregen over mezelf en waar ik echt naar op zoek ben en hoe ik mindset en zelfvertrouwen kan boosten naar de next level. 🔥"',
        name2: '- Niels D.S.',
        bg: '#1F4668',
        color: '#FFFFFF',
    },
    {
        name: '"Zeer positief verrast van Anderzom. Zeer waardevolle info die op een simpele manier wordt meegedeeld. Keep up the good work💪"',
        name2: '- Senne H.',
        bg: '#87D7FF',
        color: '#000000',
    },
    {
        name: '"Kjell en Nomi hebben uitstekend werk geleverd en ze doen dat nog steeds. Ze coachen je, staan voor je klaar, helpen je,... Ik ben nu 50 en ik voel dat ik openbloei! Exkuus is the best!"',
        name2: '- Filip A',
        bg: '#FFDF2B',
        color: '#000000',
    },
    {
        name: '"Een volledig uitgewerkt programma, dat gemakkelijk en op een verstaanbare manier te volgen is."',
        name2: '- Bryan R',
        bg: '#1F4668',
        color: '#FFFFFF',
    },
    {
        name: '"Het gaat bij Exkuus niet alleen om geld verdienen of onafhankelijk worden..."',
        name2: '- Zakia L',
        bg: '#87D7FF',
        color: '#000000',
    },
    {
        name: '"Ik raad jullie zeker aan om een kijkje te nemen of contact op te nemen met Nomi of Kjell voor een introductiegesprek."',
        name2: '- William V.D.V',
        bg: '#1F4668',
        color: '#FFFFFF',
    },
    {
        name: '"Dit is de beste investering die ik heb gedaan. Het is zo compleet en Kjell, Nomi en de hele familie staat altijd voor je klaar om je te helpen."',
        name2: '- Yasmina L.B',
        bg: '#FFDF2B',
        color: '#000000',
    },
    {
        name: '"In eerste instantie dacht ik, toch weer niet de zoveelste guru… Maar ik werd enthousiast…"',
        name2: '- Ken S',
        bg: '#1F4668',
        color: '#FFFFFF',
    },
    {
        name: '"Het Is Lang Geleden Dat Ik Iemand Ben Tegengekomen In België Die Zo Oprecht Is"',
        name2: '- Tom S.',
        bg: '#87D7FF',
        color: '#000000',
    },

    {
        name: '"Kjell Fruytier, je bent een mentor... Je bent een fantastisch voorbeeld... Je bent... gewoon jezelf en wie je bent... Dat alleen al, zorgt dat iedereen onder ons vooruit gaat. Een welgemeende bedankt om ons gisteren een shot onder de kont te geven om vooruit te gaan, om het beste uit onszelf te halen iedere dag opnieuw."',
        name2: '- Sven V.D.W.',
        bg: '#87D7FF',
        color: '#000000',
    },
    {
        name: '"Nomi, Kjell, Ilias:De volgers waarvan ik met trots mag zeggen er 1 van te zijn, BEDANKT!! Merci voor de steun wanneer deze in chat wordt gevraagd. Merci voor de hulp wanneer mijn hoofd op hol slaat en er een "?" ontstaat. Merci voor de eerlijke feedback en de handen die men uitreikt wanneer nodig."',
        name2: '- Nick P.',
        bg: '#1F4668',
        color: '#FFFFFF',
    },
    {
        name: '"Anderzom is een enorme openbaring, zeker voor iemand die totaal geen ervaring heeft met marketing. Ik heb al enorm veel geleerd en heb het gevoel dat ik al enorm ben gegroeid als persoon! Ik ben er van overtuigd dat via Anderzom en jullie support ervoor gaat zorgen dat mijn dromen werkelijkheid gaan worden! Ik ben jullie nu al enorm dankbaar! ❤️"',
        name2: '- Zoë M.',
        bg: '#FFDF2B',
        color: '#000000',
    },
    {
        name: '"Ik Volg Kjell Al Een Aantal Maanden Op Sociale Media En Ik Zag Dagelijks Zijn Consistentie, Daarom Wist Ik Dat Dit Geen Ééndagsvlieg Was"',
        name2: '- Celine M.',
        bg: '#1F4668',
        color: '#FFFFFF',
    },
];
export const videos = [
    {
        link: 'https://player.vimeo.com/video/729220382?h=6e3f5fa83b&badge=0&autopause=0&player_id=0&app_id=58479/embed',
        bg: '#FFDF2B',
        color: '#000000',
    },
    {
        link: 'https://player.vimeo.com/video/588738345?h=dc971bcb2e&amp;badge=0&amp;autopause=0&amp;player_id=0&amp;app_id=58479',
        bg: '#1F4668',
        color: '#FFFFFF',
    },
    {
        link: 'https://player.vimeo.com/video/729220405?h=d0bf1d2a15&badge=0&autopause=0&player_id=0&app_id=58479/embed',
        bg: '#87D7FF',
        color: '#000000',
    },
    {/* video nicolas & Ilias eruit
    {
        link: 'https://player.vimeo.com/video/610878536?h=38140d41df&badge=0&autopause=0&player_id=0&app_id=58479/embed',
        bg: '#275689',
        color: '#000000',
    }, 
    {
        link: 'https://player.vimeo.com/video/658580448?h=d6ffb1f469&amp;badge=0&amp;autopause=0&amp;player_id=0&amp;app_id=58479',
        bg: '#87D7FF',
        color: '#000000',
    },
    {
        link: 'https://player.vimeo.com/video/652791609?h=45f22133a0&amp;badge=0&amp;autopause=0&amp;player_id=0&amp;app_id=58479',
        bg: '#275689',
        color: '#000000',
    },*/},
    {
        link: 'https://player.vimeo.com/video/606622247?h=ddcfad18fc&amp;badge=0&amp;autopause=0&amp;player_id=0&amp;app_id=58479',
        bg: '#87D7FF',
        color: '#000000',
    },
    {/* laatste ilais vid
    {
        link: 'https://player.vimeo.com/video/731616480?h=03427ec1cb&amp;badge=0&amp;autopause=0&amp;player_id=0&amp;app_id=58479',
        bg: '#275689',
        color: '#000000',
    },*/}
];

export const screenshots = [
    { name: 'testimonial1.jpg', bg: '#FFDF2B', color: '#000000' },
    { name: 'testimonial13.jpg', bg: '#1F4668', color: '#000000' },
    { name: 'testimonial3.jpg', bg: '#87D7FF', color: '#000000' },
    { name: 'testimonial4.jpg', bg: '#275689', color: '#000000' },
    { name: 'testimonial5.jpg', bg: '#FFDF2B', color: '#000000' },
    { name: 'testimonial6.jpg', bg: '#1F4668', color: '#000000' },
    { name: 'testimonial7.jpg', bg: '#87D7FF', color: '#000000' },
    { name: 'testimonial8.jpg', bg: '#275689', color: '#000000' },
    { name: 'testimonial9.jpg', bg: '#FFDF2B', color: '#000000' },
    { name: 'testimonial10.jpg', bg: '#1F4668', color: '#000000' },
    { name: 'testimonial1.jpg', bg: '#87D7FF', color: '#000000' },
    { name: 'testimonial12.jpg', bg: '#275689', color: '#000000' },
];
